...
function NovationLaunchpad() {
    ...
    this.deviceDescription               = "Novation Launchkey MK3";        // device name
    this.needsMidiChannel                = true;                            // send midi controller to daw
    this.needsMidiBackChannel            = true;                            // send midi daw to controller
    if (getOS() == "windows")
    {
        this.midiChannelName             = "^MIDIOUT2 (LKMK3 MIDI)";        // MIDI channel name
        this.midiBackChannelName         = "^MIDIIN2 (LKMK3 MIDI)";         // MIDI channel name
    }
    else
    {
        this.midiChannelName             = "Launchkey MK3*DAW";             // MIDI channel name
        this.midiBackChannelName         = "Launchkey MK3*DAW";             // MIDI channel name
    }
    ...
    this.shutDownDevice = function() {      
        ...
        sendMidiToDevice ([0x9f, 0x0C, 0x00]); // exit DAW mode
    }
    ...
    this.onMidiReceivedFromDevice = function(msg) {
        ...
        if (d0 == 0xbf)
        {
            ...
            else if (d1 == 0x09)
            {
                ...
                else if (d2 == 7)   this.knobMode = "custom";
                else if (d2 == 8)   this.knobMode = "custom";
                else if (d2 == 9)   this.knobMode = "custom";

                triggerAsyncUpdate();
            }
            else if (d1 == 0x0a)
            {
                if (d2 == 2)        this.faderMode = "device";
                else if (d2 == 1)   this.faderMode = "volume";
                else if (d2 == 3)   this.faderMode = "pan";
                else if (d2 == 4)   this.faderMode = "send1";
                else if (d2 == 5)   this.faderMode = "send2";
                else if (d2 == 6)   this.faderMode = "custom";
                else if (d2 == 7)   this.faderMode = "custom";
                else if (d2 == 8)   this.faderMode = "custom";
                else if (d2 == 9)   this.faderMode = "custom";
            ...
            else if (d1 >= 0x35 && d1 <= 0x3c)
            {
                if (this.faderMode == "volume")
                    setFader (d1 - 0x35, d2 / 127.0, false);
                else if (this.faderMode == "pan")
                    setPanPot (d1 - 0x35, (d2 / 127.0) * 2 - 1, false);
                else if (this.faderMode == "send1")
                    setAux (d1 - 0x35, 0, d2 / 127.0);
                else if (this.faderMode == "send2")
                    setAux (d1 - 0x35, 1, d2 / 127.0);
                else if (this.faderMode == "device")
                    setParameter (d1 - 0x35, d2 / 127.0);
            }
            else if (d1 == 0x3d)
            {
        ...
        if (d0 == 0xbf)
        {
            ...
            if (! this.shift)
            {
                if (d1 == 0x73 && d2 == 0x7f)
                    play();
                else if (d1 == 0x74 && d2 == 0x7f)
                    stop();
                else if (d1 == 0x75 && d2 == 0x7f)
                    record();
                else if (d1 == 0x76 && d2 == 0x7f)
                    toggleLoop();
                else if (d1 == 0x4c && d2 == 0x7f)
                    toggleClick();
                else if (d1 == 0x4d && d2 == 0x7f)
                    undo();
            }
