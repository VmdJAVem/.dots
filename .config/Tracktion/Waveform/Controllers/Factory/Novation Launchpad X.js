...
function NovationLaunchpad() {
    ...
    this.deviceDescription               = "Novation Launchpad X";          // device name
    this.needsMidiChannel                = true;                            // send midi controller to daw
    this.needsMidiBackChannel            = true;                            // send midi daw to controller
    if (getOS() == "windows")
    {
        this.midiChannelName             = "^LPX MIDI";                     // MIDI channel name
        this.midiBackChannelName         = "^LPX MIDI";                     // MIDI channel name
    }
    else
    {
        this.midiChannelName             = "Launchpad X*DAW";               // MIDI channel name
        this.midiBackChannelName         = "Launchpad X*DAW";               // MIDI channel name
    }
    ...
    this.initialiseDevice = function() {  
        ...
        sendMidiToDevice ([0xf0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x10, 0x01, 0xf7]); // enter DAW mode
        sendMidiToDevice ([0xf0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x10, 0xf7]); // readback DAW mode
        sendMidiToDevice ([0xf0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x00, 0x00, 0xf7]); // Select session layout
    ...
    this.shutDownDevice = function() {        
        sendMidiToDevice ([0xf0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x10, 0x00, 0xf7]); // exit DAW mode
    }

    this.updateMisc = function() {
        sendMidiToDevice ([0xb0, 0x62, 0x01]);

        if (this.screen == "session")
        {
            sendMidiToDevice ([0xF0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x00, 0x00, 0xF7]);

            sendMidiToDevice ([0xb0, 0x59, 0x00]);
            sendMidiToDevice ([0xb0, 0x4F, 0x00]);
            sendMidiToDevice ([0xb0, 0x45, 0x00]);
            sendMidiToDevice ([0xb0, 0x3B, 0x00]);
            sendMidiToDevice ([0xb0, 0x31, 0x00]);
            sendMidiToDevice ([0xb0, 0x27, 0x00]);
            sendMidiToDevice ([0xb0, 0x1D, 0x00]);
            sendMidiToDevice ([0xb0, 0x13, 0x00]);
        }
        else
        {
            if (this.screen == "mixer")
            {
                if (this.mode == "pan")
                {
                    sendMidiToDevice ([0xF0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x01, 0x00, 0x00,  
                        0x00, 0x01, 0x00, 0x01,
                        0x01, 0x01, 0x01, 0x01,
                        0x02, 0x01, 0x02, 0x01,
                        0x03, 0x01, 0x03, 0x01,
                ...
                sendMidiToDevice ([0xF0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x00, 0x0D, 0xF7]);
            }
            else
            {
                sendMidiToDevice ([0xF0, 0x00, 0x20, 0x29, 0x02, 0x0C, 0x00, 0x00, 0xF7]);
            ...
            sendMidiToDevice ([0xb0, 0x59, this.mode == "volume"    ? 0x09 : 0x02]);
            sendMidiToDevice ([0xb0, 0x4F, this.mode == "pan"       ? 0x09 : 0x02]);
            sendMidiToDevice ([0xb0, 0x45, this.mode == "senda"     ? 0x09 : 0x02]);
            sendMidiToDevice ([0xb0, 0x3B, this.mode == "sendb"     ? 0x09 : 0x02]);
            sendMidiToDevice ([0xb0, 0x31, this.mode == "stopclip"  ? 0x09 : 0x02]);
            sendMidiToDevice ([0xb0, 0x27, this.mode == "mute"      ? 0x09 : 0x02]);
            sendMidiToDevice ([0xb0, 0x1D, this.mode == "solo"      ? 0x09 : 0x02]);
            sendMidiToDevice ([0xb0, 0x13, this.mode == "recordarm" ? 0x09 : 0x02]);
        }
    }    
    ...
    this.updatePads = function(rows) {
        for (var pad = 0; pad < rows; pad++)
        {
    ...
    this.updatePadArea = function() {
        ...
        else 
        {
            if (this.mode == "volume") {
                ...
                for (var track = 0; track < 8; track++)
                    sendMidiToDevice ([0xb4, track, Math.round (this.sendbCache[track] * 127)]);
            }
        }
    }
    ...
    this.onMidiReceivedFromDevice = function(msg) {
        ...
        if (d0 == 0x90)
        {
            ...
            if (clip == 7 && this.screen == "sessionB")
            {
                if (this.mode == "stopclip")
                    stopClip (track, d2 > 0);
                else if (this.mode == "solo" && d2 > 0)
                    toggleSolo (track);
                else if (this.mode == "mute" && d2 > 0)
                    toggleMute (track, false);
                else if (this.mode == "recordarm", d2 > 0)
                    toggleRecEnable (track, false);
            }
            else
            {
                launchClip (track, clip, d2 > 0);
            }
        }
        else if (d0 == 0xb0)
        {
            if (d2 == 0x7f)
            {
                ...
                {
                    if (this.screen == "mixer" || this.screen == "sessionB" || this.screen == "note" || this.screen == "custom")
                    {
                        this.screen = "session";
                    }
                    else
            ...
            {
                if (this.screen != "session" && d1 == 0x59)
                    sendMidiToDevice ([0xb0, 0x59, this.mode == "volume"    ? 0x09 : 0x02]);
                else if (this.screen != "session" && d1 == 0x4f)
                    sendMidiToDevice ([0xb0, 0x4F, this.mode == "pan"       ? 0x09 : 0x02]);
                else if (this.screen != "session" && d1 == 0x45)
                    sendMidiToDevice ([0xb0, 0x45, this.mode == "senda"     ? 0x09 : 0x02]);
                else if (this.screen != "session" && d1 == 0x3b)
                    sendMidiToDevice ([0xb0, 0x3B, this.mode == "sendb"     ? 0x09 : 0x02]);
                else if (this.screen != "session" && d1 == 0x31)
                    sendMidiToDevice ([0xb0, 0x31, this.mode == "stopclip"  ? 0x09 : 0x02]);
                else if (this.screen != "session" && d1 == 0x27)
                    sendMidiToDevice ([0xb0, 0x27, this.mode == "mute"      ? 0x09 : 0x02]);
                else if (this.screen != "session" && d1 == 0x1D)
                    sendMidiToDevice ([0xb0, 0x1D, this.mode == "solo"      ? 0x09 : 0x02]);
                else if (this.screen != "session" && d1 == 0x13)
                    sendMidiToDevice ([0xb0, 0x13, this.mode == "recordarm" ? 0x09 : 0x02]);
                else if (d1 == 0x5B || d1 == 0x5C || d1 == 0x5D || d1 == 0x5E)
