...
function AKAIAPCmini() {
    // Variables: these must be filled out so the session knows your controller layout
    this.deviceDescription               = "AKAI APC mini";          // device name
    this.needsMidiChannel                = true;                     // send midi controller to daw
    this.needsMidiBackChannel            = true;                     // send midi daw to controller

    if (getOS() == "windows")
    {
        this.midiChannelName             = "^APC mini mk2";          // MIDI channel name
        this.midiBackChannelName         = "^APC mini mk2";          // MIDI channel name
        
    }
    else
    {
        this.midiChannelName             = "APC mini*Control";       // MIDI channel name
        this.midiBackChannelName         = "APC mini*Control";       // MIDI channel name
    }
    ...
    this.initialiseDevice = function() {  
        ...
        sendMidiToDevice ([0xf0, 0x47, 0x7f, 0x4f, 0x60, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0xf7]); // intro message
    ...
    this.updateMiscLEDS = function() {
        if (this.shift)
        {
            sendMidiToDevice ([0x90, 0x64, this.faderMode  == "level"  ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x65, this.faderMode  == "pan"    ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x66, this.faderMode  == "send"   ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x67, this.faderMode  == "device" ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x68, 0x00]);
            sendMidiToDevice ([0x90, 0x69, 0x00]);
            sendMidiToDevice ([0x90, 0x6a, 0x00]);
            sendMidiToDevice ([0x90, 0x6b, 0x00]);


            sendMidiToDevice ([0x90, 0x70, this.buttonMode == "clip"   ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x71, this.buttonMode == "solo"   ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x72, this.buttonMode == "mute"   ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x73, this.buttonMode == "arm"    ? 0x01 : 0x00]);
            sendMidiToDevice ([0x90, 0x74, this.buttonMode == "select" ? 0x01 : 0x00]);
        }
        else
        {
            sendMidiToDevice ([0x90, 0x70, 0x00]);
            sendMidiToDevice ([0x90, 0x71, 0x00]);
            sendMidiToDevice ([0x90, 0x72, 0x00]);
            sendMidiToDevice ([0x90, 0x73, 0x00]);
            sendMidiToDevice ([0x90, 0x74, 0x00]);

            if (this.buttonMode == "clip")
            {
                for (var i = 0; i < 9; i++)
                    sendMidiToDevice ([0x90, 0x64 + i, this.playingCache[i] ? 0x05 : 0x00]);
            }
            else if (this.buttonMode == "solo")
            {
                for (var i = 0; i < 9; i++)
                    sendMidiToDevice ([0x90, 0x64 + i, this.solo[i]]);
            }
            else if (this.buttonMode == "mute")
            {
                for (var i = 0; i < 9; i++)
                    sendMidiToDevice ([0x90, 0x64 + i, this.mute[i]]);
            }
            else if (this.buttonMode == "arm")
            {
                for (var i = 0; i < 9; i++)
                    sendMidiToDevice ([0x90, 0x64 + i, this.arm[i]]);
            }
            else if (this.buttonMode == "select")
            {
                for (var i = 0; i < 9; i++)
                    sendMidiToDevice ([0x90, 0x64 + i, this.select[i]]);
            }
        }
    }
    ...
    this.onClipsPlayingChanged = function(channel, isPlaying) {
        ...
        {
            if (! isPlaying)
                sendMidiToDevice ([0x90, 0x64 + channel, 0x00]);
            else
                sendMidiToDevice ([0x90, 0x64 + channel, 0x05]);
        }
    }
    ...
    this.onMidiReceivedFromDevice = function(msg) {
        ...
        if (d0 == 0x90)
        {
            ...
            else if (d1 >= 0x00 && d1 <= 0x3f)
            {
                var clip = 7 - Math.floor (d1 / 8);
                var track = d1 % 8;

                launchClip (track, clip);
            }
            else if (d1 >= 0x64 && d1 <= 0x6b)
            {
                var track = d1 - 0x64;
                if (this.shift) 
                {
                    if (track == 0)
                        ...
                        changePadBanks (+1);
                    else if (track == 6 && this.faderMode == "device")
                        changeParameterBank (-1);
                    else if (track == 6)
                        changeFaderBanks (-1);
                    else if (track == 7 && this.faderMode == "device")
                        changeParameterBank (+1);
                    else if (track == 7)
                        changeFaderBanks (+1);
                }
        ...
        else if (d0 == 0xb0)
        {
            if (d1 >= 0x30 && d1 <= 0x37)
            {
                if (this.faderMode == "level")
                    setFader (d1 - 0x30, d2 / 127.0, false);
                else if (this.faderMode == "pan")
                    setPanPot (d1 - 0x30, (d2 / 127.0) * 2.0 - 1.0, false);
                else if (this.faderMode == "send")
                    setAux (d1 - 0x30, 0, d2 / 127.0);
                else if (this.faderMode == "device")
                    setParameter (d1 - 0x30, d2 / 127.0);
            }
            else if (d1 == 0x38)
            {
                if (this.faderMode == "pan")
                    setMasterPanPot ((d2 / 127.0) * 2.0 - 1.0, false);
                else
                    setMasterLevelFader (d2 / 127.0, false);
            }
        }
    }
}

registerController (new AKAIAPCmini());
