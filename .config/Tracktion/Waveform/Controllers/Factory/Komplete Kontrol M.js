...
// Calls onAsyncUpdate slightly later. Multiple calls to this function will only call onAsyncUpdate
triggerAsyncUpdate();

// Pauses for ms. Try and avoid this, but sometimes you need to wait for hardware
sleep (ms);

// Gets elapsed time, useful for calculating time-based effects
...
// Flips the visible screen
showProjectScreen();
showSettingsScreen();
showEditScreen();

// Makes huge meters on the screen
toggleBigMeterMode();
...
// Whatever keyboard shortcuts is assigned to footswitch
footSwitch1();          
footSwitch2();
...
function KompleteKontrol() {
    ...
    this.deviceDescription               = "Komplete Kontrol M";     // device name
    this.needsMidiChannel                = true;                     // send midi controller to daw
    this.needsMidiBackChannel            = true;                     // send midi daw to controller
    this.midiChannelName                 = "Komplete Kontrol M DAW"; // MIDI channel name
    this.midiBackChannelName             = "Komplete Kontrol M DAW"; // MIDI channel name
    ...
    // Triggered by triggerAsyncUpdate()
