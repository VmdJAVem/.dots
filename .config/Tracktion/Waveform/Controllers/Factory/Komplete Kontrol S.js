...
function KompleteKontrol() {
    ...
    this.deviceDescription               = "Komplete Kontrol S";       // device name
    this.needsMidiChannel                = true;                       // send midi controller to daw
    this.needsMidiBackChannel            = true;                       // send midi daw to controller
    this.midiChannelName                 = "Komplete Kontrol DAW - 1"; // MIDI channel name
    this.midiBackChannelName             = "Komplete Kontrol DAW - 1"; // MIDI channel name
    ...
    this.metersL                         = new Array();                // Meter levels
    this.metersR                         = new Array();                // Meter levels
    this.metersDirty                     = false;                      // Meter levels are dirty, resend next chance you get
    this.lastSelected                    = 0;                          // Last channel to have selection
        ...
        if (! this.connected) {
            ...
            if (name == "meterTimer") {
                if (this.metersDirty) {
                    ...
                    for (var j = 0; j < this.numberOfFaderChannels; j++) {
                        var l = this.gainToPos(this.metersL[j]);
                        var r = this.gainToPos(this.metersR[j]);
    ...
    this.solveLine = function (x1, y1, x2, y2, x) {
        var m = (y2 - y1) / (x2 - x1);
        var b = y1 - (m * x1);
    ...
    this.gainToPos = function(gain) {
        var db = Math.log10 (gain) * 20;
        if (db <= -100) return 0;
        if (db >= 12)   return 127;

        if (db < -48) return Math.round (this.solveLine (-100,  0, -48,  16, db));
        if (db < -24) return Math.round (this.solveLine (-48,  16, -24,  38, db));
        if (db < -12) return Math.round (this.solveLine (-24,  38, -12,  68, db));
        if (db < 0)   return Math.round (this.solveLine (-12,  68,   0, 109, db));
        if (db < 12)  return Math.round (this.solveLine ( 0,  109,  12, 127, db));
    ...
    // tells the device to move the master fader, if it has them. 
    // slider pos is 0 to 1.0
    ...
    this.onChannelLevelChanged = function(channel, left, right) {   
        if (this.metersL[channel] != left) {
            this.metersL[channel] = left;      
            this.metersDirty = true;
        }
        if (this.metersR[channel] != right) {
            this.metersR[channel] = right;      
            this.metersDirty = true;
        }
    }
    ...
    this.onMidiReceivedFromDevice = function(msg) {
        if (msg[0] == 0xbf) {
            if (msg[1] == 0x01) {
                ...
                toggleRecEnable (msg[1], false);
            }
            else if (msg[1] >= 0x50 && msg[1] <= 0x57) {
                var w = msg[2] <= 63 ? msg[2] : -(128 - msg[2]);
                setFader (msg[1] - 0x50, w / 2048, true);
            }
                ...
                var w = msg[2] <= 63 ? msg[2] : -(128 - msg[2]);
                setFader (this.lastSelected, w / 2048, true);
            }
            else if (msg[1] == 0x65) {
                var w = msg[2] <= 63 ? msg[2] : -(128 - msg[2]);
                setPanPot (this.lastSelected, w / 2048, true);
            }
            else if (msg[1] == 0x66) {
                toggleMute (this.lastSelected);
            }
            else if (msg[1] == 0x67) {
                toggleSolo (this.lastSelected);
            }
        }
    }
